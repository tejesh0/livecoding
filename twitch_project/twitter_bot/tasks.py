...
from .models import LivecodingHandle
...
def like_and_retweet_livecoding_mentions():
    ...
    for result in LivecodingHandle.objects.all():
        search_results = api.search(q=result.twitter_handle)
        ...
        logger.info("liked and retweeted tweets consisting" + result.twitter_handle + "term")
...
def retweet_and_like_following_account_tweets(q):
    ...
    for friend_id in friends_ids:
        ...
        for status in statuses:
            # check if tweet has 20 retweets
            if status._json['retweet_count'] > 20 and not status._json['retweeted'] and retweet_count < RETWEET_LIMIT:
                try:
